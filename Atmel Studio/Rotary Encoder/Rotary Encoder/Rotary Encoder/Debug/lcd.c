...
#define lcd_command_direction_port DDRD
...
void lcd_port_config (void)
{
	...
	lcd_data_direction_port = lcd_data_direction_port | 0xF0; //all the LCDs data pin's direction set as output (1111 0000)
	//lcd_data_port = lcd_data_port & 0xF0; // all the LCDs data pins are set to logic 0
	
	lcd_command_direction_port = lcd_command_direction_port | 0b11100000;
	//lcd_command_port = lcd_command_port | 0b10100000;
	
	//cbit(lcd_command_port,RW);
}
...
static void lcd_wr_command(unsigned char cmd)
{
	...
	//temp = temp<<2;
	lcd_data_port &= 0x0F;		//Clear output 0000 1111
	//lcd_data_port &= 0xC3;		//Clear output 1100 0011
	
	lcd_data_port |= temp;		//Output upper nibble
	
	cbit(lcd_command_port,RS);
	sbit(lcd_command_port,EN);
	_delay_ms(5);
	cbit(lcd_command_port,EN);
	...
	lcd_data_port |= cmd;		//Output lower nibble
